# Replace the regular convolutional layer of ResNet to spherenet
...
def sphere_conv3x3(in_planes, out_planes, stride=1):
    """3x3 sphere convolution with padding"""
    return SphereConv2D(in_planes, out_planes, kernel_size=3, stride=stride, bias=False)
...
class SphereBottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.conv1 = SphereConv2D(inplanes, planes, kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm2d(planes)
        self.conv2 = SphereConv2D(planes, planes, kernel_size=3, stride=stride, bias=False)
        self.bn2 = nn.BatchNorm2d(planes)
        self.conv3 = SphereConv2D(planes, planes * 4, kernel_size=1, bias=False)
...
class SphereResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000):
        ...
        self.conv1 = SphereConv2D(3, 64, kernel_size=7, stride=2, bias=False)
        self.bn1 = nn.BatchNorm2d(64)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = SphereMaxPool2D(kernel_size=3, stride=2)
...
def sphere_resnet50(pretrained=False, **kwargs):
    """Constructs a SphereResNet-50 model.
